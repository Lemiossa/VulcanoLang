...
Value eval(AstNode *root) {
  ...
  case NODE_PROGRAM: {
    for (size_t i = 0; i < root->data.program.count; i++) {
      v = eval(root->data.program.statements[i]);
    }
  } break;
  case NODE_BLOCK_STATEMENT: {
    for (size_t i = 0; i < root->data.blockStatement.count; i++) {
      Value tmp = eval(root->data.blockStatement.statements[i]);
  ...
  case NODE_EXPRESSION_STATEMENT: {
    return eval(root->data.expressionStatement.expression);
  } break;
  case NODE_NUMBER: {
  ...
  case NODE_BINARYOP: {
    Value left = eval(root->data.binaryOp.left);
    Value right = eval(root->data.binaryOp.right);
